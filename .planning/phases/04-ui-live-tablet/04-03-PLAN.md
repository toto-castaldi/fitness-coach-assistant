...
Component exports GroupExerciseCard.
  ...
  <done>GroupExerciseCard component created with all required functionality.</done>
</task>

<task type="auto">
  <name>Task 3: Create GroupExerciseView component and integrate in TabletLive</name>
  <files>src/live/components/GroupExerciseView.tsx, src/live/pages/TabletLive.tsx</files>
  <action>
**Part A: Create GroupExerciseView.tsx**
...
2. Add undo function to restore state after complete-all undo:
   ```typescript
   // Add to component body:
   const handleUndoGroupComplete = async (exerciseIds: string[]) => {
     // Refetch to get authoritative state after undo
     // The RPC doesn't have an undo, so we need to update each individually
   ...
   Import supabase at top: `import { supabase } from '@/shared/lib/supabase'`

3. Replace the placeholder in the main content conditional:
   ```typescript
   {viewMode === 'individual' ? (
     <>
       <ActionPanel ... />
...
4. Ensure the main content area has proper flex layout for both views.
  </action>
  <verify>
`npm run build` succeeds.
Dev server:
- Toggle to group view shows group exercises
- "Completa tutti" button works with toast
- Skip individual works
- Undo toast action reverts completion
  </verify>
  <done>
...
- Skip individual per client works
